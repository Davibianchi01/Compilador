...
function parser(Lexico $lexico, $debug = false): bool {
    // Define a gramática e a tabela LR (AFD)
    ...
    // Imprime estado atual e token lido
    echo "Estado: $s | Token: $tok | Lexeme: " . ($lookahead['lexeme'] ?? 'null') . "\n";
    ...
    if ($action[0] === 'S') {
        ...
        echo "Shift -> estado " . end($stateStack) . "\n";
        $lookahead    = null;
        continue;
    }
    ...
    if ($action[0] === 'R') {
        ...
        $stateStack[] = $goto;
        $valStack[]   = $lhsAttr;
        echo "Reduce usando regra R$rid -> $lhsSym | Próximo estado: $goto\n";
        continue;
    }
    ...
    if ($action === 'ACC') {
        ...
        echo "Compilação semântica OK. (Aceito)\n";
        return true;
    }
